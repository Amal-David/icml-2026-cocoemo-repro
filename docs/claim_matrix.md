...
Frozen public proxy: [`claim4-cremad-mixed-directional.yaml`](../configs/claim4-cremad-mixed-directional.yaml) selects exactly one supported multi-rater disagreement item from each of 60 deterministically SHA-256-ranked CREMA-D actors. Our proxy-only stratification is explicit, not paper-authored: supported disagreement, positive neutral mass, at least two active non-neutral labels, and a unique maximum among angry/happy/sad votes. The release-pinned vote table (`1658cd3...`, SHA-256 `774d...377b`) contains 7,442 audio rows and yields exactly 118 such items from 60 actors. Every selected target has a distinct deterministic same-actor neutral reference whose filename label is neutral and whose released vote table has neutral as the unique majority. Target and reference WAVs are acquired individually from the pinned Git revision only after their Git-LFS pointer SHA-256 and byte size verify, then atomically cached without republishing source audio.

Every selected sample has seven arms: released unrenormalized four-way mixing, norm-matched dominant non-neutral, a deterministic no-fixed-point label shuffle with norm matching, sample-seeded norm-matched random, non-neutral-renormalization diagnostic, explicit-zero-neutral diagnostic, and zero-steering identity control. The full protocol therefore requires exactly `60 x 7 = 420` WAVs. Vector files, SHA-256 hashes, `attn_output` layer 17, shape `[1, 896]`, CosyVoice2 revision, finite output, 22,050 Hz sample rate, and maximum clipping fraction `0.001` are all hard contracts.

The first executable GPU step is intentionally only one actor x seven arms in [`claim4-cremad-mixed-canary.yaml`](../configs/claim4-cremad-mixed-canary.yaml). It hash-locks the full 60x7 configuration and checks CUDA, the pinned CosyVoice source/model preparation, LFS target/reference acquisition, vector injection, output hashes, and complete seven-WAV integrity accounting; it is recorded as `runtime_canary_passed`, never Claim 4 evidence. A full evaluator job is allowed only after that canary is recorded. Its frozen model locks are Emotion2Vec `emotion2vec/emotion2vec_plus_large@6c303ba987b86b93193de93e34bb2b077a6bedc4`, WavLM `microsoft/wavlm-base-sv@0a23162ffc49adcf42bdf836a00cb2eb45af3601` (speaker similarity against source reference, not an emotion classifier), and Whisper `openai/whisper-large-v3@06f233fe06e710322aca913c1bc4249a0d71fce1`.

The preregistered full-proxy decision uses 10,000 actor-cluster paired bootstrap resamples: released mixing must improve both proportion Spearman rho and H-rate against both shuffled and norm-matched random controls, be non-worse than the norm-matched dominant arm on rho, preserve WavLM speaker similarity, and stay within a `0.02` WER increase versus the identity arm. A confident reverse or quality breach fails; otherwise the result is inconclusive. This alpha-5-only proxy deliberately makes no alpha-5 versus alpha-3 claim. None of these automatic metrics establishes N-MOS.
